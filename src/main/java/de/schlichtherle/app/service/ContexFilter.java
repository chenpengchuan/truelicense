...
public class ContexFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest request, ServletResponse response1, FilterChain chain) throws IOException, ServletException {
        ...
        if(httpServletRequest.getRequestURI().equals("/")){
            response.sendRedirect("/license/index.html");
            return;
        }

        if((httpServletRequest.getRequestURI().startsWith("/api/license")||
                httpServletRequest.getRequestURI().startsWith("/api"))&&
                !httpServletRequest.getRequestURI().equals("/api/auth/login")){
